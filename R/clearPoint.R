...
#'
#' @export
clearPoint <- function(dataFrame, UTMZone, distLength){
  # Specify the coordinate X and Y from a data-frame
  points <- dataFrame[, c("X", "Y")]

  # Convert to spatial points
  points_sp <- sp::SpatialPoints(points,proj4string=CRS(as.character(UTMZone)))

  # Create distance matrix between points
  points_sp_dist <- rgeos::gDistance(points_sp, byid = T)

  # Define the distance threshold
  points_sp_dist_mtr <- rgeos::gWithinDistance(points_sp, byid = TRUE,
                                               dist=distLength)
